"""Tests for the Blender LDraw render wrapper (unit + optional integration)."""
...
def test_env_kill_switch(monkeypatch):
    ...
    assert ldraw_render.is_available() is False
...
def test_blender_env_override(monkeypatch, tmp_path):
    ...
    assert ldraw_render.find_blender() == str(fake)


def test_render_unavailable_raises(monkeypatch, tmp_path):
    monkeypatch.setattr(ldraw_render, "find_blender", lambda: None)
    with pytest.raises(ldraw_render.LDrawRenderError, match="unavailable"):
        ldraw_render.render_ldraw("x.ldr", str(tmp_path / "out.png"))
...
def test_stitch_combines_views_horizontally(tmp_path):
    ...
    ldraw_render._stitch(paths, str(out))
